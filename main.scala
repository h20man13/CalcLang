...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    var shouldntBreak: Boolean = true
    var promptSymbol: String = "CalcLang>"
    while(shouldntBreak){
      val line: String = readLine(promptSymbol + " ")
      if(!line.isEmpty){
        if(line(0) == ':'){
          if(line.length == 1){
            println("Error: expected command but found nothing")
          } else {
            val command: String = line.substring(1)
            command match{
              case "Help" => helpCommand()
              case "Quit" => shouldntBreak = false
              case "Enviro" => {
                printHeader("Variables: ")
                println(Passes.variables.toString())
                printHeader("Functions: ")
                println(Passes.functions.toString())
              }
              case "Variables" => println(Passes.variables.toString())
              case "Functions" => println(Passes.functions.toString())
              case cmd if cmd.startsWith("Prompt") => {
                val data: Array[String] = cmd.split(" ")
                if(data.length != 2){
                  println("Prompt symbols must contain no whitespace or tabs -> " + cmd)
                } else {
                  promptSymbol = data(1)
                }
              }
              case a => println("Unknown command " + a)
            }
          }
        } else {
          ...
          val stat: Statement = pser.parseStatement()
          Passes.interpret(stat)
        }
      }
    }
  }
